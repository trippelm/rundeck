...
public class NotificationService implements ApplicationContextAware{
    ...
    void asyncTriggerJobNotification(String trigger, schedId, Map content){
        if(trigger && schedId){
            if(grailsApplication.config.rundeck.notifications.own.thread in [true,'true']){
                Thread.start {
                    Thread notificationThread = new NotificationThread(this, trigger, schedId, content)
                    notificationThread.start()
                    if(!grailsApplication.config.rundeck.notifications.timeout.trim().isEmpty()){
                        notificationThread.join(Long.parseLong(grailsApplication.config.rundeck.notifications.timeout))
                    }
